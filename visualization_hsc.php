...
$hsc_data = mysqli_query($con, "SELECT * FROM hsc_edu_data ORDER BY Enroll_id ASC");

$num_rows = mysqli_num_rows($hsc_data);
$chart_data = array();

if ($num_rows > 0) {
    $counter = 1; // Start counter from 1
    while ($row = mysqli_fetch_assoc($hsc_data)) {
        $percentage = min(max($row['H_percentage'], 0), 100);
        $percentile = min(max($row['H_percentile'], 0), 100);
...
$board_data = mysqli_query($con, "SELECT H_board, COUNT(*) as count FROM hsc_edu_data GROUP BY H_board");
...
while ($row = mysqli_fetch_assoc($board_data)) {
    ...
    if ($row['H_board'] == 'CBSE') {
        ...
        $modified_chart_data_board[] = array($row['H_board'], intval($row['count']));
    }
}
...
$grade_data = mysqli_query($con, "SELECT H_grade, COUNT(*) as count FROM hsc_edu_data GROUP BY H_grade");
...
while ($row = mysqli_fetch_assoc($grade_data)) {
    ...
    $chart_data_grade[] = array($row['H_grade'], intval($row['count']));
}
?>
<?php
...
$combined_percentage_query = mysqli_query($con, "SELECT H_percentage FROM hsc_edu_data ORDER BY Enroll_id ASC");
...
$combined_percentile_query = mysqli_query($con, "SELECT H_percentile FROM hsc_edu_data ORDER BY Enroll_id ASC");
...
<head>
    ...
    <title>Performance Comparison hsc</title>
...
<body>
    ...
    <center>
        ...
        <div>
            <p>If you want to change/update data then please <a href="educationaldetails_hsc_edit.php"><span style="color: blue;">update</span></a> your details .</p>
        </div>
    </center>
    <div class="div-header">
        <h1>
            HSC Student Performance Analysis
        </h1>
